...
* query(query, options) - This will trigger a GET request to {target}{query}. If query is an object, it will be serialized using URL encoding. If query is a string it is passed in the URL as-is. If options includes a sort property, it will be serialized as a query parameter as well (see `Sorting`_ for more information).
