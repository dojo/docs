...
* :ref:`dojo/request/notify` - Publishes the ``dojo/request`` topic for requests.

* :ref:`dojo/request/watch` - Provides the capability to watch in-flight requests.

* :ref:`dojo/request/registry` - Allows registering different providers by a URI pattern, so that different providers
  can be used based on the target URI without the user having to code separate code paths.
