...
|                      |          |         |the height is inherited from its offset parent (which is the node's parent if the position of the parent is not "static"). If "auto" is specified, the content    |
