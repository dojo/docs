...
* **io**:  Modules dealing with the reading and writing of ATOM data.
  ...
  * **Connection.js**:  A module that wrappers the JavaScript model for an ATOM feed and provides several helper functions for getting feeds, updating entries, creating new entries, and deleting entries.  Under the covers it uses model.js and the `dojo.xhr <dojo/_base/xhr>`_ functions to implement the full APP (ATOM Publishing Protocol).
