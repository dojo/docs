...
Decoupled concepts of "focused node" and "selected node", so that:
   ...
   1. A tree doesn't start with any node mark as selected.   App needs to call attr("selectedItem", ...) or attr("path", ...) to mark the currently selected node.  It's unclear what should happen when the user clicks a node (especially when clicking a branch node that merely causes the node to open).  For now, clicking a node does not select it.
