...
For more details on Dijit themes (Claro, Tundra, Nihilo and Soria) and theming, see `Themes and Theming <http://docs.dojocampus.org/dijit-themes>`_
...
* `dojox.editor.plugins.FindReplace <dojox/editor/plugins/FindReplace>`_ -- This plugin was updated to improve its layout and keyboard interaction based on input from a user experience development team.  Feature-wise it remains the same, but the input and button layout is improved.
