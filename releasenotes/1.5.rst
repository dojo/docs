...
---------------------
   - New 'Map Chart' widget ( dojox.charting.Map ) provides an easy way to display geographical map data.  ( eg: USA, Europe, Asia etc... )
   - Improved support for gradient/pattern fills 
   ...
   - Several incremental enhancements to the DataPresentation widget, including support for custom tooltips, layout/wrapping options for the chart legend, ability to supress the chart labels  and 'hooks' into the rendering.
