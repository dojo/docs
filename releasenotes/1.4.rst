...
ContentPane
-----------
Fixes related to resizing and href loading, including avoid premature href loading for ContentPanes in nested TabContainers.  ContentPane now acts as a full layout widget, meaning that it calls resize() on it's child widgets when (not before) it is made visible, so no problems with ContentPane's children being initialized while hidden.
...
* Path and selectedItem attributes to get/set the current tree item (`#9339 <http://bugs.dojotoolkit.org/ticket/9339>`_)
...
--------------------
Keyboard support added, users can select a date in the calendar using arrow keys, and page-down/up for month navigation.
