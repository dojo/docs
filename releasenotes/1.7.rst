...
The parser also supports the ability to specify dojo.on and object.watch declaratively by adding the support for script types "dojo/on" and "dojo/watch".

An example of "dojo/on":
...
An example of "dojo/watch" (building on "dojo/on"):
...
Note, there is no support for deprecated "prop" as an attribute of the "dojo/watch" script tag.  The parser only recognises "data-dojo-prop".
...
- Gridx is an incubation project provided as a basis for the next generation of Dojo Grid. 
- Gridx provides a highly extensible core and a rich set of modules to be loaded on demand, appropriate modules can be picked up to constitute different Grid instances e.g. MiniGrid(MobileGrid）, AdvancedGrid, TreeGrid etc.
- Gridx is `demo only <http://archive.dojotoolkit.org/dojo-2011-07-18/dojotoolkit/demos/gridx/demo.html>`_ and not for production use in 1.7 
