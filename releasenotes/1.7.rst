...
Mobile (dojox/mobile)
  ...
  * Android 2.2, 2.3, 3.0 
  * Blackberry 6 
  * Mobile compatibility on desktop browsers: IE 8, 9, Firefox 3.6, 4, 5, Safari 5.0, 5.1, Chrome 13 and later
...
Based on dojo.touch and dojo.on, this new module provides a mechanism to write gestures working well on difference devices including desktop (single gestures) and various touch devices. 
...
- Provide common tap gestures(single touch only) including tap, tap.hold and tap.doubletap
- Customizable settings e.g. threshhold for tap.hold, effective radius for a valid tap.doubletap
 
dojox/gesture/swipe

- Provide common swipe gestures(single touch only) including swipe, swipe.end

Also `a quick demo <http://archive.dojotoolkit.org/dojo-2011-07-18/dojotoolkit/demos/touch/demo.html>`_ shows dijit/form/HorizontalSlider and dojo/dnd can now work well on iOS4+ with the new dojo/touch and dojo/gesture. Besides a tap gesture, the demo also shows how easy it is to write a new rotate gesture(multiple touch).
 
Please refer to `dojo/gesture doc(TODO) <dojo/gesture>`_ for more details.
...
- An incubation project being worked closely with 'dgrid<https://github.com/kriszyp/dgrid>'_ for the next generation of Dojo Grid.
- Provides a highly extensible core and a rich set of modules to be loaded on demand, appropriate modules can be picked up to constitute different Grid instances e.g. MiniGrid(MobileGrid）, AdvancedGrid, TreeGrid etc.
- Gridx is `demo only <http://archive.dojotoolkit.org/dojo-2011-07-18/dojotoolkit/demos/gridx/demo.html>`_ in 1.7 time frame.
