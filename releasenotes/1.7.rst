...
--------
  ...
  - Bidi text support has been added through two BidiSupport classes (one for dojox.charting, one for dojox.charting.widget) to require in your application in order for Bidi text support to be activated.
...
- You may need to explicitly require the following modules which were getting implicitly loaded before:
   ...
   * dijit.form.DropDownButton, dijit.form.ComboButton, dijit.form.ToggleButton (formerly in dijit/form/Button)
   * dijit.form.MappedTextBox, dijit.form.RangeBoundTextBox (formerly in dijit/form/ValidationTextBox)
   * dijit.form.RadioButton (formerly in dijit/form/Checkbox)
   * dijit.MenuItem, dijit.PopupMenuItem, dijit.CheckedMenuItem, dijit.MenuSeparator (previously pulled in by dijit.Menu)
   * dijit.TooltipDialog (previously pulled in by dijit.Dialog)
   * dijit.layout.AccordionPane (previously pulled in by AccordionContainer).  Or better yet, instead of requiring AccordionPane, just use dijit.layout.ContentPane instead.
   * dijit.ToolbarSeparator (previously pulled in by dijit.Toolbar)
