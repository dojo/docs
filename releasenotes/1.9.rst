...
*Features*
   ...
   * dojox/app has also changed the default location used for the “controller” if it is not specified for a view in the config.  Now dojox/app will attempt to find the “controller” with the same name and folder as the “template” file.
   * dojox/app now provides "has" test support for the config file, so there is no longer a need for multiple configs for an app.  In the example below "phone" and "ie9orLess" would be set with has.add("phone", !isTablet); and has.add("ie9orLess", has("ie") && (has("ie") <= 9)); and dojox/app/main would process the has sections and merge the appropriate sections into the config.  Properties set on the config at the same level as the has will be replaced by the value set in the has section, and items in an array will be added to config array with the same name and at the same level as the has section. 
