...
*Features*
   ...
   * The 'Custom' theme is now generated from 2 main colors, facilitating the creation of new theme based on arbitrary colors.
...
*Incompatibilities*
   ...
   * ``dojox/mobile/sniff`` module does not anymore require ``dojo/_base/sniff`` but ``dojo/sniff`` this means you can't use anymore ``isXX`` methods (e.g. isIE) you should use has flags instead (e.g. has("ie")).
...
*Features*
   ...
   * Charting bidi features (bi-directional text and mirroring) can be enabled using the dojo-bidi has flag you should not use BidiSupport modules anymore.
