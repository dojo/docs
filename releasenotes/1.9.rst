...
*Features*
   ...
   * It is no longer necessary to set a view controller to "none" when the view does not use a controller, now when view does not have a controller set no controller will be loaded.
...
*Incompatibilities*
   ...
   * The option to be able to load a default controller for a view (when the controller is not set) has been removed, the controller must be specified for a view in order to be loaded. If the view does not have a controller module to load, it should not set a controller, setting the controller to "none" is no longer supported.
