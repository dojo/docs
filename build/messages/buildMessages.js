...
fs.readFile("template.rst", "utf-8", function(err, template){
	messages.forEach(function(message){
		var output = template.replace(/%msg_no%/gi, message[1]);
		output = output.replace(/%msg_msg%/gi, message[2]);
		output = output.replace(/%msg_txt%/gi, message[3]);
		switch(true){
			case(message[1] >= 100 && message[1] < 200):
				output = output.replace(/%msg_type%/gi, "Information");
				break;
			case(message[1] >= 200 && message[1] < 300):
				output = output.replace(/%msg_type%/gi, "Warning");
				break;
			case(message[1] >= 300 && message[1] < 400):
				output = output.replace(/%msg_type%/gi, "Error");
				break;
			case(message[1] >= 400 && message[1] < 500):
				output = output.replace(/%msg_type%/gi, "Report");
				break;
			default:
				output = output.replace(/%msg_type%/gi, "Unknown");
		}
		fs.writeFile(message[1] + ".rst", output, "utf-8");
	});
});
