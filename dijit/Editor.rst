...
Prior to dojo 1.4, the Editor would default its RichText frame to whatever the browser decided for an iframe, and this could be problematic when it comes to showing UTF-8 characters and the like.  So, in 1.4, the editor has been updated so that it will inherit the charset from the page loading the editor (from dojo.doc), which means it will be in the same charset as the rest of the page.  This should help eliminate issues with conflicing charactersets between the containing page and the RTE.  However, should you desire that the editor run in a different character set than the rest of the page, you can still do that.  Simply set the property 'charset' when declaring the editor declaratively or programatically to the character set you desire.  The underlying frame in the RTE will then use that character set for its document.
